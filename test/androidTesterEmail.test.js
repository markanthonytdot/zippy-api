...
test('Android branded invitation uses Internal Testing and does not contain a credential', () => {
  ...
  for (const value of [email.html, email.text]) {
    assert.ok(value.includes(ANDROID_TEST_URL)); assert.match(value, /Account → Partner Preview/);
  ...
  assert.match(email.html, /cid:zippi-bunny/); assert.equal(email.attachments.length, 1);
});
