...
async function main() {
  ...
  app.use('/v1/demo-feedback',createDemoFeedbackPublicRouter({service,allowedOrigins:'http://localhost:4318,http://localhost:4173,http://127.0.0.1:4173'}));
  app.get(['/admin','/admin/'],(_req,res)=>res.redirect('/admin/demo-feedback'));
  app.use('/admin',createAdminDashboardRouter({dbPool,adminSecret:process.env.DEMO_FEEDBACK_DEV_ADMIN_SECRET,sessionSecret:secret,demoFeedbackService:service}));
  ...
  const server=app.listen(4318,'127.0.0.1',()=>console.log('LOCAL ONLY: http://localhost:4318/demo-feedback/ and http://localhost:4318/admin/demo-feedback'));
