...
async function main() {
  ...
  const ios = { configured: true, async enroll(email) {
    ...
    return { testerId: "local-fixture", state: email.startsWith("pending") ? "NOT_INVITED" : "INVITED" };
  }, async reconcile() { return { testerId: "local-fixture", state: "INVITED" }; }, async refresh() { return { state: "ACCEPTED" }; } };
  ...
  const worker = require("../lib/testerInvitationWorker").startTesterInvitationWorker(invitations);
  ...
  for (const signal of ["SIGINT", "SIGTERM"]) process.on(signal, () => server.close(async () => { clearInterval(worker); await pool.end(); await setup.query(`drop schema ${schema} cascade`); await setup.end(); process.exit(0); }));
}
