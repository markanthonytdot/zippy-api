(() => {
  ...
  function render(data) {
    ...
    const entryName = key => key === 'flight_thank_you' ? 'Flight thank-you' : 'Direct';
    for (const [selector,values] of [['#usefulness',data.overall.usefulness],['#entries',data.overall.entryPaths]]) {
      const parent=document.querySelector(selector); parent.replaceChildren();
      for (const [key,value] of Object.entries(values)) {
        const row=el('div'); row.append(el('dt',selector==='#entries' ? entryName(key) : key),el('dd',`${value.count} · ${value.percent}%`)); parent.append(row);
      }
    }
    const funnel=document.querySelector('#funnel'); funnel.replaceChildren();
    const f=data.funnel;
    metric(funnel,'Flight feedback completed',f.flightCompleted,'Flight responses');
    metric(funnel,'Clicked “Watch the package demo”',f.packageClicks,'Deduplicated click events');
    metric(funnel,'Package submitted from flight flow',f.fromFlightSubmitted,'Flight thank-you entry');
    metric(funnel,'Click-through rate',f.clickThroughPercent===null ? '—' : `${f.clickThroughPercent}%`,`${f.packageClicks} clicks / ${f.flightCompleted} flight responses`);
    metric(funnel,'Completion after click',f.completionAfterClickPercent===null ? '—' : `${f.completionAfterClickPercent}%`,`${f.fromFlightSubmitted} package responses / ${f.packageClicks} clicks`);
    ...
    const names=['Date / time','Source','Entry path','Likelihood to use','More useful than separate search','Additional comments'];
    for (const row of data.rows) {
      const tr=el('tr');
      const values=[new Date(row.submitted_at).toLocaleString(),sourceName(row.source),entryName(row.entry_path),row.likelihood,row.usefulness,row.comment || '—'];
      values.forEach((value,index)=>{
        const td=el('td'); td.dataset.label=names[index];
        if (index===5 && value.length>180) {
          const details=el('details'); details.append(el('summary',value.slice(0,140)+'… Read full answer'),el('p',value)); td.append(details);
        } else td.append(el('p',value,index===5 ? 'answer-text' : undefined));
        tr.append(td);
      }); tbody.append(tr);
    }
  ...
  async function load() {
    ...
    try {
      const response = await fetch(`/admin/api/package-feedback?${query}`, { cache:'no-store' });
  ...
  exportButton.addEventListener('click',async () => {
    ...
    try {
      const response = await fetch(`/admin/api/package-feedback/export.csv?${applied}`, { cache:'no-store' });
      if (response.status === 401) { location.assign('/admin/login'); return; }
      ...
      link.href=url; link.download=`zippi-package-feedback-${new Date().toISOString().slice(0,10)}.csv`; document.body.append(link); link.click(); link.remove();
