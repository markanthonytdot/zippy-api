...
function parseFilters(query = {}) {
  ...
  for (const key of ['source', 'likelihood', 'recent', 'from', 'to', 'sort', 'page', 'survey_version', 'clarity', 'booked']) {
    const value = query[key];
    if (value === undefined || value === '') continue;
    if (typeof value !== 'string') throw new FeedbackError(400, 'invalid_filter');
    out[key] = value;
  }
  if (out.survey_version && !['combined_demo_v3', 'free_text_v1', 'multiple_choice_v2'].includes(out.survey_version)) throw new FeedbackError(400, 'invalid_survey_version');
  if (out.clarity && !['Very clear', 'Somewhat clear', 'Not very clear', 'Not clear at all'].includes(out.clarity)) throw new FeedbackError(400, 'invalid_clarity');
  if (out.booked && !['yes', 'no'].includes(out.booked)) throw new FeedbackError(400, 'invalid_booked_filter');
  if ((out.clarity || out.booked) && out.survey_version !== 'combined_demo_v3') throw new FeedbackError(400, 'invalid_filter');
  if (out.recent && out.survey_version === 'combined_demo_v3') throw new FeedbackError(400, 'invalid_filter');
...
function filterSql(filters) {
  ...
  if (filters.survey_version === 'free_text_v1') clauses.push('comprehension_choice is null');
  if (filters.survey_version === 'multiple_choice_v2') clauses.push('comprehension_choice is not null');
...
function createDemoFeedbackService({ dbPool, secret, now = Date.now }) {
  const { createCombinedDemoFeedbackService, surveyTotals, EXTRA_HEADERS } = require('./combinedDemoFeedback');
  const combined = createCombinedDemoFeedbackService({ dbPool, parseFilters, csvHeaders: CSV_HEADERS });
  ...
  async function submit(body, ip) {
    ready(); await rateLimit(ip);
    if (body && Object.hasOwn(body, 'survey_version')) {
      if (body.survey_version !== 'combined_demo_v3') throw new FeedbackError(400, 'invalid_survey_version');
      return combined.submit(body);
    }
    const data = validateSubmission(body);
  ...
  async function report(query) {
    ready(); const filters = parseFilters(query);
    if (filters.survey_version === 'combined_demo_v3') return combined.report(query);
    ...
    try {
      ...
      const rows = (await client.query(`select ${filters.survey_version ? 'request_id,' : ''}${FIELDS} from demo_feedback_responses ${where} order by submitted_at ${dir},id ${dir} limit ${PAGE_SIZE} offset $${params.length + 1}`, [...params, (filters.page - 1) * PAGE_SIZE])).rows;
      ...
      const versionInfo = filters.survey_version ? { surveyVersion: filters.survey_version, totals: await surveyTotals(client, filters) } : {};
      if (filters.survey_version) for (const row of rows) row.survey_version = row.comprehension_choice ? 'multiple_choice_v2' : 'free_text_v1';
      await client.query('commit');
      return { ok: true, ...versionInfo, ...summary, rows, sourceOptions: sources, page: filters.page, pages, pageSize: PAGE_SIZE };
    } catch (error) { await client.query('rollback'); throw error; } finally { client.release(); }
  }
  async function* exportCsv(query) {
    ready(); const filters = parseFilters(query);
    if (filters.survey_version === 'combined_demo_v3') { yield* combined.exportCsv(query); return; }
    const { where, params } = filterSql(filters);
    const client = await dbPool.connect(); let complete = false;
    try {
      await client.query('begin isolation level repeatable read read only');
      const dir = filters.sort === 'oldest' ? 'asc' : 'desc';
      await client.query(`declare feedback_export no scroll cursor for select ${filters.survey_version ? 'request_id,' : ''}${FIELDS} from demo_feedback_responses ${where} order by submitted_at ${dir},id ${dir}`, params);
      yield '\uFEFF' + (filters.survey_version ? [...CSV_HEADERS, ...EXTRA_HEADERS] : CSV_HEADERS).map(csvCell).join(',') + '\r\n';
      while (true) {
        const { rows } = await client.query('fetch forward 500 from feedback_export');
        if (!rows.length) break;
        yield rows.map(row => filters.survey_version ? csvLine(row).slice(0, -2) + ',' + [row.comprehension_choice ? 'multiple_choice_v2' : 'free_text_v1', row.request_id, '', '', '', '', '', ''].map(csvCell).join(',') + '\r\n' : csvLine(row)).join('');
      }
