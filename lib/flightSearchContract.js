...
function rankOffers(offers) {
  const cheapest = offers.slice().sort((left, right) => {
    ...
    const leftAmount = left.totalAmount == null ? Infinity : Number(left.totalAmount);
    const rightAmount = right.totalAmount == null ? Infinity : Number(right.totalAmount);
  ...
  const finitePrices = offers.map((offer) => Number(offer.totalAmount)).filter((value, index) =>
  ...
  const best = offers.slice().sort((left, right) => {
    ...
    const leftScore = normalizedPenalty(left.totalAmount == null ? NaN : Number(left.totalAmount), minPrice, maxPrice) * 55
      + normalizedPenalty(left.totalDurationMinutes, minDuration, maxDuration) * 45;
    const rightScore = normalizedPenalty(right.totalAmount == null ? NaN : Number(right.totalAmount), minPrice, maxPrice) * 55
...
function buildFlightSearchContract(rawOffers, requestedCurrency) {
  const offers = [];
  ...
  const rankingCurrency = rankingCurrencyForOffers(offers, requestedCurrency);
  const rankedOffers = offers.map((offer) => ({
    ...offer,
    rankingEligibility: {
      best: Boolean(rankingCurrency && offer.totalCurrency === rankingCurrency && offer.totalAmount != null
        && Number.isFinite(offer.totalDurationMinutes) && offer.totalDurationMinutes >= 0),
      cheapest: Boolean(rankingCurrency && offer.totalCurrency === rankingCurrency && offer.totalAmount != null),
  ...
  return {
    ...
    rankingAvailability,
    offers: rankedOffers,
    rankings: rankOffers(rankedOffers),
  };
}
