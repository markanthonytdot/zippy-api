...
const SAFE_ERRORS = new Set(['android_testers_unavailable', 'invalid_android_action', 'invitation_not_found',
  'invalid_email', 'invalid_duration', 'invalid_expiry', 'expiry_changed', 'extension_must_increase_expiry',
...
function input(body) {
  return { action: body?.action, platform: body?.platform, id: body?.id,
    ...
    ...(['prepare', 'extend'].includes(body?.action) ? { durationDays: body?.durationDays } : {}),
    ...(body?.action === 'extend' ? { expectedExpiresAt: body?.expectedExpiresAt } : {}) };
}
