...
const SAFE_ERRORS = new Set(['android_testers_unavailable', 'invalid_android_action', 'invitation_not_found',
  'invalid_email', 'invalid_adjustment', 'invalid_duration', 'invalid_expiry', 'expiry_changed', 'extension_must_increase_expiry',
  'email_domain_not_allowed', 'invalid_organization', 'preview_access_inactive',
...
function input(body) {
  return { action: body?.action, platform: body?.platform, id: body?.id,
    ...
    ...(['prepare', 'extend', 'adjust'].includes(body?.action) ? { durationDays: body?.durationDays } : {}),
    ...(['extend', 'adjust'].includes(body?.action) ? { expectedExpiresAt: body?.expectedExpiresAt } : {}),
    ...(body?.action === 'adjust' ? { operation: body?.operation } : {}) };
}
