...
const FILTERS = new Set(['survey_version', 'clarity', 'booked', 'source', 'likelihood', 'recent', 'usefulness', 'entry_path', 'from', 'to', 'sort', 'page']);
...
function feedbackNavigation(html, active = '') {
  if (active === 'package-feedback') html = html.replace('<title>Package demo feedback', '<title>Historical package demo feedback').replace('<h1>Package demo feedback</h1>', '<h1>Historical package demo feedback</h1>');
  html = html.replace(/<a\b[^>]*href="\/admin\/(?:demo-feedback|package-feedback)"[^>]*>[\s\S]*?<\/a>/g, '');
  const links = [['demo-feedback', 'Demo feedback', 'D'], ['package-feedback', 'Package feedback (historical)', 'P']]
