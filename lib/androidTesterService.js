...
const ACTIONS = new Set(['prepare', 'confirm', 'unconfirm', 'removed', 'send', 'retry', 'resend', 'revoke', 'disable']);
...
function project(row, now) {
  return { id: row.id, email: row.email, platform: 'android', authority: 'production',
    access: accessState(row, 'android', now), expiresAt: row.expires_at,
...
function createAndroidTesterService({ dbPool, access, mail, secret, enabled = false,
  ...
  const policy = /^[0-9a-f-]{36}$/i.test(organizationId) && [3, 7, 14].includes(durationDays);
  ...
  async function list() {
    const config = { enabled: !!enabled, policyConfigured: policy, emailConfigured: !!mail?.configured,
      authority: 'production', durationDays, testingUrl: ANDROID_TEST_URL };
  ...
  async function run(input, actor) {
    ...
    try {
      ...
      let person;
      if (action === 'prepare') {
        person = await access.ensureTesterAccess({ email, platform: 'android', organizationId, durationDays }, actor, client);
