...
const ACTIONS = new Set(['prepare', 'confirm', 'unconfirm', 'removed', 'send', 'retry', 'resend', 'revoke', 'disable', 'extend']);
...
function createAndroidTesterService({ dbPool, access, mail, secret, enabled = false,
  ...
  async function run(input, actor) {
    ...
    if (['prepare', 'extend'].includes(action) && !validAccessDuration(selectedDays))
      throw new PartnerAccessError(400, 'invalid_duration');
    if (action === 'extend' && (input.confirm !== true || typeof input.expectedExpiresAt !== 'string'))
    ...
    try {
      ...
      if (['confirm', 'unconfirm', 'removed'].includes(action)) {
        ...
        // Dedicated, audited action; repeated/stale submissions cannot extend twice.
        await access.changePerson(row.person_id, 'extend', { durationDays: selectedDays,
          expectedExpiresAt: input.expectedExpiresAt }, actor);
        await read(); // Extension never restores revoked/disabled access or changes Play/email state.
