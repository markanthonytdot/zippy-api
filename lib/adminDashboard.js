...
function createAdminDashboardRouter(options = {}) {
  ...
  function sendAdminPage(res, filename) {
    if (!demoFeedbackService) return res.sendFile(path.join(publicDirectory, filename));
    const html = fs.readFileSync(path.join(publicDirectory, filename), "utf8").replace(
      '<p class="nav-label">Business</p>',
      '<p class="nav-label">Business</p><a href="/admin/demo-feedback"><span class="nav-icon">D</span><span>Demo feedback</span></a>'
    );
  ...
  if (demoFeedbackService) {
    router.get("/demo-feedback", (_req, res) => res.sendFile(path.join(publicDirectory, "demo-feedback.html")));
    router.get("/assets/demo-feedback.js", (_req, res) => res.sendFile(path.join(publicDirectory, "demo-feedback.js")));
    router.get("/assets/demo-feedback.css", (_req, res) => res.sendFile(path.join(publicDirectory, "demo-feedback.css")));
