const { createAndroidTesterAdminRouter } = require("./androidTesterRoutes");
const crypto = require("node:crypto");
const fs = require("node:fs");
const { createDemoFeedbackAdminRouter } = require("./demoFeedbackRoutes");
const { createPackageFeedbackAdminRouter } = require("./packageFeedbackRoutes");
...
function createAdminDashboardRouter(options = {}) {
  ...
  function sendAdminPage(res, filename) {
    if (!demoFeedbackService && !packageFeedbackService) return res.sendFile(path.join(publicDirectory, filename));
    let html = fs.readFileSync(path.join(publicDirectory, filename), "utf8");
    if (packageFeedbackService && filename === "demo-feedback.html") html = html
      .replace('<title>Demo feedback', '<title>Flight demo feedback')
      .replace('<h1>Demo feedback</h1>', '<h1>Flight demo feedback</h1>')
      .replace('<span>Demo feedback</span>', '<span>Flight demo</span>');
    const links = [];
    if (demoFeedbackService && !html.includes('href="/admin/demo-feedback"')) links.push(`<a href="/admin/demo-feedback"><span class="nav-icon">D</span><span>${packageFeedbackService ? "Flight demo" : "Demo feedback"}</span></a>`);
    if (packageFeedbackService && !html.includes('href="/admin/package-feedback"')) links.push('<a href="/admin/package-feedback"><span class="nav-icon">P</span><span>Package demo</span></a>');
    html = html.replace('<p class="nav-label">Business</p>', '<p class="nav-label">Business</p>' + links.join(''));
  ...
  // Old feedback bookmarks land on the established tester-admin host. No cookie,
  // user-controlled return URL, or credential crosses origins in this redirect.
  if (feedbackAdminCanonical) router.get(["/demo-feedback", "/package-feedback"], (req, res) => {
    const origin = require("./feedbackReadBridge").FEEDBACK_ADMIN_ORIGIN;
    return res.redirect(302, origin + "/admin" + req.path.replace(/\/$/, ""));
  });
  ...
  if (demoFeedbackService) {
    router.get("/demo-feedback", (_req, res) => sendAdminPage(res, "demo-feedback.html"));
    router.get("/assets/demo-feedback.js", (_req, res) => res.sendFile(path.join(publicDirectory, "demo-feedback.js")));
    router.use("/api/demo-feedback", createDemoFeedbackAdminRouter({ service: demoFeedbackService }));
  }
  if (demoFeedbackService || packageFeedbackService) router.get("/assets/demo-feedback.css", (_req, res) => res.sendFile(path.join(publicDirectory, "demo-feedback.css")));
  if (packageFeedbackService) {
    router.get("/package-feedback", (_req, res) => sendAdminPage(res, "package-feedback.html"));
    for (const file of ["package-feedback.js", "package-feedback.css"]) router.get(`/assets/${file}`, (_req, res) => res.sendFile(path.join(publicDirectory, file)));
    router.use("/api/package-feedback", createPackageFeedbackAdminRouter({ service: packageFeedbackService }));
  }
  if (androidTesterService) {
    router.use("/api/android-testers", createAndroidTesterAdminRouter({ service: androidTesterService }));
    for (const file of ["android-testers.js", "android-testers.css"]) router.get(`/assets/${file}`, (_req, res) => res.sendFile(path.join(publicDirectory, file)));
  }
  router.get("/partner-access", (_req, res) => sendAdminPage(res, "partner-access.html"));
  for (const file of ["partner-access.css", "tester-invitations.css"]) router.get(`/assets/${file}`, (_req, res) => res.sendFile(path.join(publicDirectory, file)));
  router.get("/assets/tester-access-duration.js", (_req, res) => res.sendFile(path.join(publicDirectory, "tester-access-duration.js")));
  ...
  router.get(["/", "/overview", "/flights", "/currencies", "/flights/bookings", "/hotels", "/hotels/bookings"], (_req, res) => {
    return sendAdminPage(res, "index.html");
  });
  return router;
}
