...
const { feedbackNavigation } = require("./feedbackAdminRemote");
const { createAndroidTesterAdminRouter } = require("./androidTesterRoutes");
const crypto = require("node:crypto");
const path = require("node:path");
const express = require("express");
const { createPartnerAdminRouter } = require("./partnerAccessRoutes");
const { createTesterInvitationRouter } = require("./testerInvitationRoutes");
const {
  buildPricingQuote,
  calculatePricing,
  currencyExponent,
...
function createAdminDashboardRouter(options = {}) {
  ...
  function sendAdminPage(res, filename) {
    if (!feedbackAdmin?.enabled) return res.sendFile(path.join(publicDirectory, filename));
    const html = fs.readFileSync(path.join(publicDirectory, filename), "utf8");
    return res.type("html").send(feedbackNavigation(html, filename.replace(".html", "")));
  }
  ...
  if (feedbackAdmin?.enabled) {
    for (const name of ["demo-feedback", "package-feedback"]) {
      router.get("/" + name, (_req, res) => sendAdminPage(res, name + ".html"));
      for (const extension of ["js", "css"]) router.get(`/assets/${name}.${extension}`, (_req, res) => res.sendFile(path.join(publicDirectory, `${name}.${extension}`)));
    }
    router.use("/api", feedbackAdmin.router);
  }
  ...
  if (partnerAccessService) {
    router.get("/partner-access", (_req, res) => sendAdminPage(res, "partner-access.html"));
    router.get("/assets/partner-access.js", (_req, res) => res.sendFile(path.join(publicDirectory, "partner-access.js")));
    router.get("/assets/partner-access.css", (_req, res) => res.sendFile(path.join(publicDirectory, "partner-access.css")));
    router.use("/api/partner-access", createPartnerAdminRouter({ service: partnerAccessService }));
    if (testerInvitationService) {
      router.get("/assets/tester-invitations.js", (_req, res) => res.sendFile(path.join(publicDirectory, "tester-invitations.js")));
      router.get("/assets/tester-invitations.css", (_req, res) => res.sendFile(path.join(publicDirectory, "tester-invitations.css")));
      router.use("/api/tester-invitations", createTesterInvitationRouter({ service: testerInvitationService }));
    }
  }
