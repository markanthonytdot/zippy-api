    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 409 && path === "betaTesterInvitations") {
          ...
          if (problem?.errors?.some(error => error.code === "STATE_ERROR.TESTER_INVITE.NO_INSTALLABLE_BUILDS")) throw failure("apple_no_testable_build");
        }
    ...
    for (let offset = 0; offset < ids.length; offset += 50) {
      ...
      for (const build of builds.data) {
        ...
        if (batch.includes(build.id) && build.attributes?.expired === false && new Date(build.attributes?.expirationDate).getTime() > now()
          // Approval with notifications held is not installable: Apple's invite
          // endpoint rejects it with TESTER_INVITE.NO_INSTALLABLE_BUILDS.
          && detail?.attributes?.externalBuildState === "IN_BETA_TESTING") eligible.push(detail.attributes);
      }
    }
