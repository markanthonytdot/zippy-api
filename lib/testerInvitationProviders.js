...
function createAppleTesterProvider(env = process.env, { fetchImpl = globalThis.fetch, now = Date.now } = {}) {
  ...
  async function preflight() {
    ...
    const builds = await collection(`betaGroups/${groupId}/builds?limit=200&include=buildBetaDetail`);
    const eligible = builds.data.filter(build => {
      ...
      return build.attributes?.expired === false && new Date(build.attributes?.expirationDate).getTime() > now()
        && detail?.attributes?.externalBuildState === "IN_BETA_TESTING";
    });
    if (!eligible.length) throw failure("apple_no_testable_build");
    return { autoNotify: eligible.some(build => builds.included.find(item => item.id === build.relationships.buildBetaDetail.data.id)?.attributes?.autoNotifyEnabled === true) };
  }
  ...
  return {
    ...
    async enroll(email, { reserveNotification }) {
      ...
      tester = (await request(`betaTesters/${id}`))?.data;
    ...
    async refresh(testerId) {
      return project((await request(`betaTesters/${resourceId(testerId)}`))?.data);
    },
    async resend(testerId, { reserveNotification }) {
      ...
      if (!memberships.data.some(group => group.id === groupId)) throw failure("apple_membership_unconfirmed");
      const state = project((await request(`betaTesters/${testerId}`))?.data);
      ...
      return project((await request(`betaTesters/${testerId}`))?.data);
    },
  };
}
