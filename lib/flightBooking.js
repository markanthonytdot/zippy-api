...
function sessionPricingQuote(session) {
  ...
  return buildPricingQuote({
    ...
    customerRawBaseFareMinor: Number(session.customer_base_fare_minor ?? session.base_fare_minor ?? providerBaseFareMinor),
    customerRawTaxesMinor: session.customer_taxes_minor == null ? taxesMinor : Number(session.customer_taxes_minor),
    customerRawServicesMinor: Number(session.customer_services_minor ?? session.services_minor ?? 0),
