...
function validateQuotePayload(payload) {
  ...
  return { ...payload, offerId, returnOfferId: returnOfferId || null };
}
...
function calculatePricing(offer, services, zippiFeeMinor) {
  ...
  let taxesMinor = offer?.tax_amount == null ? null : decimalToMinor(offer.tax_amount, currency);
  const baseFareMinor = offer?.base_amount == null
    ? (taxesMinor == null ? offerMinor : offerMinor - taxesMinor)
    : decimalToMinor(offer.base_amount, currency);
  if (taxesMinor == null && offer?.base_amount != null && baseFareMinor !== offerMinor) {
    taxesMinor = offerMinor - baseFareMinor;
  }
  if (baseFareMinor < 0 || (taxesMinor != null && (taxesMinor < 0 || baseFareMinor + taxesMinor !== offerMinor))) {
...
function buildPricingQuote(pricing) {
  const required = [
    pricing.baseFareMinor,
    pricing.offerMinor,
    pricing.servicesMinor,
    pricing.duffelTotalMinor,
    pricing.zippiFeeMinor,
    pricing.chargeTotalMinor,
  ];
  if (!required.every((value) => Number.isSafeInteger(value) && value >= 0)
      || (pricing.taxesMinor != null && (!Number.isSafeInteger(pricing.taxesMinor) || pricing.taxesMinor < 0))
      || (pricing.taxesMinor != null && pricing.baseFareMinor + pricing.taxesMinor !== pricing.offerMinor)
      || pricing.offerMinor + pricing.servicesMinor !== pricing.duffelTotalMinor
      || pricing.duffelTotalMinor + pricing.zippiFeeMinor !== pricing.chargeTotalMinor) {
    throw new FlightBookingError(500, "invalid_quote", "The server produced an inconsistent checkout quote.");
  }
  return {
    currency: pricing.currency,
    lineItems: {
      baseFareMinor: pricing.baseFareMinor,
      taxesMinor: pricing.taxesMinor,
      servicesMinor: pricing.servicesMinor,
      zippiFeeMinor: pricing.zippiFeeMinor,
    },
    offerTotalMinor: pricing.offerMinor,
    duffelTotalMinor: pricing.duffelTotalMinor,
...
function sessionPricingQuote(session) {
  const offerMinor = Number(session.offer_minor);
  ...
  const taxesMinor = persistedBase == null
    ? (snapshot.taxAmount == null ? null : decimalToMinor(snapshot.taxAmount, session.currency))
    : persistedTaxes;
  const baseFareMinor = persistedBase ?? (snapshot.baseAmount == null
    ? (taxesMinor == null ? offerMinor : offerMinor - taxesMinor)
    : decimalToMinor(snapshot.baseAmount, session.currency));
  return buildPricingQuote({
    currency: session.currency,
    baseFareMinor,
    taxesMinor,
    offerMinor,
    servicesMinor: Number(session.services_minor),
    duffelTotalMinor: Number(session.duffel_total_minor),
    zippiFeeMinor: Number(session.zippi_fee_minor),
    chargeTotalMinor: Number(session.charge_total_minor),
  });
}
...
function createFlightBookingService(options) {
    ...
    try {
      ...
      const result = await client.query(
        `insert into flight_booking_sessions (
         ...
         ) values ($1,$2,$3,$4,$5::jsonb,$6::jsonb,$7::jsonb,$8::jsonb,$9::jsonb,$10,$11,$12,$13,$14,$15,$16,$17,'not_created','payment_setup')
         returning *`,
        [
          ...
          pricing.baseFareMinor, pricing.taxesMinor, pricing.offerMinor, pricing.servicesMinor, pricing.duffelTotalMinor,
          pricing.zippiFeeMinor, pricing.chargeTotalMinor,
        ]
      );
      await client.query("commit");
  ...
  async function getBookingStatus(userId, bookingSessionId) {
    ...
    const staleDisposition = staleBookingClaimDisposition(session, Date.now(), bookingClaimLeaseMs);
    if (staleDisposition === "safe_retry") {
      const recovered = await dbPool.query(
        `update flight_booking_sessions
            set booking_status = 'awaiting_payment', recovery_status = 'safe_retry_ready',
                failure_code = 'stale_pre_order_claim', failure_message = 'A previous booking attempt stopped before airline submission and can be retried.',
                updated_at = now()
          where id = $1 and user_id = $2 and booking_status = 'booking_in_progress'
            and duffel_post_started_at is null and updated_at < now() - ($3::bigint * interval '1 millisecond')
          returning *`,
        [id, userId, bookingClaimLeaseMs]
      );
      if (recovered.rows[0]) session = recovered.rows[0];
    } else if (staleDisposition === "outcome_unknown") {
      const unknown = await dbPool.query(
        `update flight_booking_sessions
            set booking_status = 'booking_unknown', recovery_status = 'manual_reconciliation_required',
                failure_code = 'stale_post_attempt', failure_message = 'The airline order attempt needs reconciliation.', updated_at = now()
          where id = $1 and user_id = $2 and booking_status = 'booking_in_progress'
            and duffel_post_started_at is not null and updated_at < now() - ($3::bigint * interval '1 millisecond')
          returning *`,
        [id, userId, bookingClaimLeaseMs]
      );
      if (unknown.rows[0]) session = unknown.rows[0];
    }
    ...
    if (session.stripe_payment_intent_id && ["payment_setup", "awaiting_payment", "payment_paid"].includes(session.booking_status)) {
      ...
      const paymentMatches = intent.amount === Number(session.charge_total_minor)
        && normalizeCurrency(intent.currency) === session.currency;
  ...
  async function claimSession(userId, bookingSessionId) {
    ...
    if (current.booking_status === "booking_in_progress") {
      const disposition = staleBookingClaimDisposition(current, Date.now(), bookingClaimLeaseMs);
        ...
        const unknown = await dbPool.query(
          `update flight_booking_sessions
            ...
            where id = $1 and user_id = $2 and booking_status = 'booking_in_progress'
              and duffel_post_started_at is not null and updated_at < now() - ($3::bigint * interval '1 millisecond')
            returning *`,
          [bookingSessionId, userId, bookingClaimLeaseMs]
        );
        if (unknown.rows[0]) {
          throw new FlightBookingError(409, "booking_outcome_unknown", "The airline booking outcome is still being reconciled. Do not try again.");
        }
      ...
      if (disposition === "safe_retry") {
        const reclaimed = await dbPool.query(
          `update flight_booking_sessions
              set duffel_attempted_at = now(), updated_at = now(), failure_code = null, failure_message = null
            where id = $1 and user_id = $2 and booking_status = 'booking_in_progress'
              and duffel_post_started_at is null and updated_at < now() - ($3::bigint * interval '1 millisecond')
            returning *`,
          [bookingSessionId, userId, bookingClaimLeaseMs]
        );
        if (reclaimed.rows[0]) return { confirmed: false, session: reclaimed.rows[0] };
        throw new FlightBookingError(409, "booking_in_progress", "This booking is already being processed. Do not submit it again.");
      }
    }
  ...
  async function refundAfterDefinitiveFailure(session, code, message, duffelRequestId = null) {
    const marked = await dbPool.query(
      `update flight_booking_sessions
        ...
        returning *`,
      [session.id, code, message, duffelRequestId]
    );
    if (!marked.rows[0]) {
    ...
    try {
      await updateFailure(session.id, completed ? "booking_failed_refunded" : "booking_failed_refund_pending", code, message, {
  ...
  async function confirmBooking(userId, bookingSessionId) {
    ...
    const postMarker = await dbPool.query(
      `update flight_booking_sessions
        ...
        where id = $1 and user_id = $2 and booking_status = 'booking_in_progress' and duffel_post_started_at is null
        returning *`,
      [session.id, userId]
    );
