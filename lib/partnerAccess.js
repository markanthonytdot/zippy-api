...
function projectPerson(person, now) {
  return {
    ...
    status: person.status, access: accessState(person, null, now),
...
function createPartnerAccessService({ dbPool, secret, signToken, mailAdapter, now = Date.now } = {}) {
  ...
  async function requestCode(input) {
    ...
    await transaction(async client => {
      ...
      const id = crypto.randomUUID(); const code = mail.development && mail.fixedCode ? mail.fixedCode : String(crypto.randomInt(0, 1000000)).padStart(6, "0");
      ...
      await client.query(`insert into partner_verifications(id,person_id,code_digest,platform,expires_at,created_at) values($1,$2,$3,$4,$5,$6)`,
        [id, person.id, codeDigest(secret, id, code), platform, iso(now() + CODE_TTL_SECONDS * 1000), iso(now())]);
      try { await mail.send({ email, code, expiresInSeconds: CODE_TTL_SECONDS }); }
      catch {
  ...
  async function verifyCode(input) {
    ...
    const result = await transaction(async client => {
      ...
      if (!/^\d{6}$/.test(String(input.code)) || !equalDigest(verification.code_digest, codeDigest(secret, verification.id, String(input.code)))) return null;
  ...
  function expiry(input, start) {
    const end = input.expiresAt != null ? new Date(input.expiresAt).getTime() : [3, 7, 14].includes(input.durationDays) ? start + input.durationDays * 86400000 : NaN;
  ...
  return { requestCode, verifyCode, status, list, createOrganization, createPerson, changePerson, deleteAccount };
}
