...
function createPartnerAccessService({ dbPool, secret, signToken, mailAdapter, reviewerAccess = null, now = Date.now } = {}) {
  ...
  async function ensureTesterAccess({ email: rawEmail, platform: rawPlatform, organizationId, durationDays = 7 }, actor, existingClient = null) {
    ...
    return transaction(async client => {
      ...
      if (person) {
        ...
        const end = expiry({ durationDays }, now());
        const id = crypto.randomUUID();
        await client.query(`insert into partner_people(id,email,organization_id,starts_at,expires_at,platforms,features,created_at,updated_at)
          values($1,$2,$3,$4,$5,$6,$7,$4,$4)`, [id, email, org.id, iso(now()), end, JSON.stringify([platform]), DEFAULT_FEATURES]);
  ...
  return { requestCode, verifyCode, status, list, createOrganization, createPerson, changePerson, ensureTesterAccess, deleteAccount };
}
