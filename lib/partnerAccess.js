...
const { REVIEW_DIGEST_PREFIX } = require("./partnerAccessReviewer");
...
function createPartnerAccessService({ dbPool, secret, signToken, mailAdapter, reviewerAccess = null, now = Date.now } = {}) {
  ...
  async function changePerson(id, action, input, actor) {
    return transaction(async client => {
      ...
      if (action === "adjust") {
        if (!["set", "extend"].includes(input.operation)) throw new PartnerAccessError(400, "invalid_adjustment");
        if (!validAccessDuration(input.durationDays) || input.expiresAt != null) throw new PartnerAccessError(400, "invalid_duration");
        if (input.confirm !== true || typeof input.expectedExpiresAt !== "string") throw new PartnerAccessError(400, "manual_confirmation_required");
        if (input.expectedExpiresAt !== iso(person.expires_at)) throw new PartnerAccessError(409, "expiry_changed");
        const timestamp = now();
        const base = input.operation === "set" ? timestamp : Math.max(timestamp, new Date(person.expires_at).getTime());
        const end = expiry({ durationDays: input.durationDays }, base);
        if (Date.parse(end) <= Date.parse(person.starts_at)) throw new PartnerAccessError(400, "invalid_expiry");
        // Only expiry changes. Revocation, disabled status, sessions and distribution stay intact.
        await client.query("update partner_people set expires_at=$2,expiry_audited_at=null,updated_at=$3 where id=$1", [id, end, iso(timestamp)]);
        await audit(client, person, "access_adjusted", actor, { operation: input.operation, durationDays: input.durationDays,
          previousExpiresAt: iso(person.expires_at), expiresAt: end, adjustedAt: iso(timestamp) });
