...
const ROUTE_ERRORS = new Set([...SAFE_ERRORS, "invalid_email", "invalid_platform", "tester_policy_not_configured", "tester_invitations_disabled",
  "email_domain_not_allowed", "invitation_not_found", "invitation_in_progress", "rate_limited", "tester_qa_restricted"]);
function createTesterInvitationRouter({ service }) {
  ...
  router.post("/", route(req => service.run({ email: req.body?.email, platform: req.body?.platform }, req.zippiAdmin.actor, "invite", context())));
