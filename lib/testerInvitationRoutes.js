...
const ROUTE_ERRORS = new Set([...SAFE_ERRORS, "invalid_email", "invalid_platform", "tester_policy_not_configured", "tester_invitations_disabled",
  "email_domain_not_allowed", "invitation_not_found", "invitation_in_progress", "rate_limited"]);
function createTesterInvitationRouter({ service }) {
  ...
  router.post("/", route(req => service.run({ email: req.body?.email, platform: req.body?.platform }, req.zippiAdmin.actor)));
  for (const action of ["retry", "resend", "apple-resend", "refresh"]) {
    router.post(`/:id/${action}`, route(req => service.run({ id: req.params.id }, req.zippiAdmin.actor, action)));
  }
  return router;
}
