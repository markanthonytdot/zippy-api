...
const ROUTE_ERRORS = new Set([...SAFE_ERRORS, "invalid_email", "invalid_platform", "invalid_duration", "tester_policy_not_configured", "tester_invitations_disabled",
...
function createTesterInvitationRouter({ service }) {
  ...
  router.post("/", route(req => service.run({ email: req.body?.email, platform: req.body?.platform, organizationId: req.body?.organizationId, durationDays: req.body?.durationDays }, req.zippiAdmin.actor, "invite", context())));
