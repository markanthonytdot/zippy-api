...
const PRIVATE_PAGES_TEST_ORIGIN = "https://main.heyzippi-website-test.pages.dev";

function normalizePrivateStagingOrigins(origins) {
  if (!origins.has(BARE_PAGES_TEST_ORIGIN)) return origins;
  const normalized = new Set(origins);
  normalized.delete(BARE_PAGES_TEST_ORIGIN);
  normalized.add(PRIVATE_PAGES_TEST_ORIGIN);
  return normalized;
}
