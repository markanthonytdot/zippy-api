...
function createTesterInvitationService({ dbPool, partnerAccessService, providers, mailAdapter, secret, env = process.env, now = Date.now }) {
  ...
  function ready() {
    if (!dbPool || !secret || !enabled) throw new PartnerAccessError(503, "tester_invitations_disabled");
  }
  async function list() {
    const config = { enabled, policyConfigured, durationDays: policyConfigured ? durationDays : null, emailConfigured,
      platforms: { ios: !!providers.ios?.configured, android: !!providers.android?.configured } };
    if (!enabled || !dbPool) return { ok: true, config, invitations: [] };
    const rows = (await dbPool.query(`${SELECT} order by i.updated_at desc limit 200`)).rows;
  ...
  async function run(input, actor, action = "invite") {
    ready();
