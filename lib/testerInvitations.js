...
function project(row, now) {
  return { id: row.id, email: row.email, organizationId: row.organization_id, organization: row.organization,
    platform: row.platform, access: row.status === "disabled" || row.organization_status === "disabled" ? "disabled" : accessState(row, row.platform, now),
...
function createTesterInvitationService({ dbPool, partnerAccessService, providers, mailAdapter, secret, env = process.env, now = Date.now }) {
  ...
  const policyConfigured = /^[0-9a-f-]{36}$/i.test(String(organizationId || "")) && validAccessDuration(durationDays);
  ...
  async function list(context) {
    ...
    const config = { enabled, policyConfigured, durationDays: policyConfigured ? durationDays : null, durationOptions: ACCESS_DURATION_DAYS, durationMinDays: 1, durationMaxDays: 90, emailConfigured,
  ...
  async function run(input, actor, action = "invite", context) {
    ...
    let email, platform;
    const selectedDays = input.durationDays === undefined ? durationDays : input.durationDays;
    if (action === "invite" && !validAccessDuration(selectedDays)) throw new PartnerAccessError(400, "invalid_duration");
    if (action === "invite") {
    ...
    try {
      ...
      const person = action === "invite" ? await partnerAccessService.ensureTesterAccess({ email, platform, organizationId, selectedOrganizationId, durationDays: selectedDays }, actor, client) : null;
