...
const SELECT = `select i.*,p.email,p.organization_id,p.status,p.revoked_at,p.platforms,p.starts_at,p.expires_at,
  o.status as organization_status from tester_invitations i join partner_people p on p.id=i.person_id
...
function project(row, now) {
  return { id: row.id, email: row.email, platform: row.platform, access: accessState(row, row.platform, now),
...
function createTesterInvitationService({ dbPool, partnerAccessService, providers, mailAdapter, secret, env = process.env, now = Date.now }) {
  ...
  const qaConfigured = !enabled && env.ZIPPI_TESTER_QA_ENABLED === "true"
    && env.RENDER_SERVICE_ID === "srv-dagq12ht0dsc73a7dm40"
    && env.ZIPPI_TESTER_APPLE_APP_ID === "6757395108"
    && env.ZIPPI_TESTER_APPLE_GROUP_ID === QA_GROUP;
  ...
  async function list(context) {
    const qaOnly = qaAllowed(context) ? { email: QA_EMAIL, platform: "ios" } : null;
    const config = { enabled, policyConfigured, durationDays: policyConfigured ? durationDays : null, emailConfigured,
      qaOnly, platforms: { ios: !!providers.ios?.configured, android: !qaOnly && !!providers.android?.configured } };
  ...
  async function run(input, actor, action = "invite", context) {
    ...
    if (!enabled && (email !== QA_EMAIL || platform !== "ios")) throw new PartnerAccessError(403, "tester_qa_restricted");
    ...
    try {
      ...
      const person = await partnerAccessService.ensureTesterAccess({ email, platform, organizationId, durationDays }, actor, client);
      ...
      try {
        if (action === "refresh" || action === "apple-resend") {
          ...
          const result = action === "refresh" ? await providers.ios.refresh(row.provider_tester_id)
            : await providers.ios.resend(row.provider_tester_id, { reserveNotification });
          ...
          if (row.platform_status !== "confirmed") {
            let result;
            try {
              result = await providers[platform].enroll(email, { reserveNotification });
...
module.exports = { createTesterInvitationService, SAFE_ERRORS };
