...
function validateReconciledOrder(order, session) {
  ...
  if (!session?.id || !session.stripe_payment_intent_id || !session.duffel_offer_id
      || !session.currency || session.duffel_total_minor == null) return false;
  let orderTotalMinor;
  try {
  ...
  return order.live_mode === false
    ...
    && String(order.total_currency || "").toUpperCase() === String(session.currency || "").toUpperCase()
    && String(orderTotalMinor) === String(session.duffel_total_minor)
    ...
    && optionalMetadataMatches("zippi_currency", session.currency, (value) => String(value).toUpperCase())
    && optionalMetadataMatches("zippi_duffel_total_minor", session.duffel_total_minor);
}
...
function validateRefundForSession(refund, session) {
  ...
  return Boolean(refund?.id)
    ...
    && String(refund.currency || "").toUpperCase() === String(session?.currency || "").toUpperCase()
    && Number.isSafeInteger(refund.amount)
    && String(refund.amount) === String(session?.charge_total_minor)
