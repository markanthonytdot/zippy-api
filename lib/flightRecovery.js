...
async function repairRecoveryJobs(dbPool, { leaseMs = 5 * 60 * 1000, bookingMode = null } = {}) {
  const result = await dbPool.query(
    `insert into flight_booking_recovery_jobs (booking_session_id, kind)
     ...
     where ((booking_status = 'booking_in_progress'
       ...
       and ($2::text is null or coalesce(booking_mode, 'test') = $2)
     on conflict (booking_session_id, kind) do update
...
async function claimRecoveryJobs(dbPool, {
  ...
  try {
    ...
    const result = await client.query(
      `with candidates as (
         select jobs.booking_session_id, jobs.kind
         from flight_booking_recovery_jobs jobs
         join flight_booking_sessions sessions on sessions.id = jobs.booking_session_id
         where ((jobs.status = 'queued' and jobs.available_at <= now())
            or (jobs.status = 'claimed' and jobs.claim_expires_at <= now()))
           and ($5::text is null or coalesce(sessions.booking_mode, 'test') = $5)
       ...
       returning jobs.*`,
      [limit, token, String(workerId || "flight-recovery-worker"), leaseMs, bookingMode]
    );
...
function validateReconciledOrder(order, session) {
  ...
  const expectedLiveMode = String(session.booking_mode || "test").toLowerCase() === "live";
  return order.live_mode === expectedLiveMode
...
function createFlightRecoveryService({
  ...
  async function runOnce({ workerId, limit = 1, claimLeaseMs = 60_000 } = {}) {
    await repairRecoveryJobs(dbPool, { leaseMs, bookingMode });
    const jobs = await claimRecoveryJobs(dbPool, { workerId, limit, leaseMs: claimLeaseMs, bookingMode });
    const results = [];
